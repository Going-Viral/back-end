...
describe('mobility data', () => {
  ...
  it('fetchs all data for a single country by code, including subregions', () => {
    const countryCode = 'AE';
    return request(app)
  ...
  it('fetch all data for a single subregion1, excluding subregion2s', () => {
    const countryCode = 'CA';
    return request(app)
      .get(`/api/v1/mobility/subRegion/${countryCode}/test`)
      .then(res => {
        expect(res.body).toContainEqual({
          _id: expect.any(String),
          date: expect.any(String),
  ...
  it('fetch all data for a single subregion1, including subregion2s', () => {
